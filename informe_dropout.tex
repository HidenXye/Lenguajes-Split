...
% =============================================
\newpage
\section{Redes Bayesianas -- Tres Enfoques DAG}

Se implementaron y compararon tres enfoques para la construcción de Redes Bayesianas (Grafos Acíclicos Dirigidos, DAG) sobre el dataset de abandono universitario. Cada enfoque produce una estructura de dependencias diferente, y se evalúa su calidad mediante el criterio de información Bayesiana (BIC):

\begin{equation}
    \text{BIC} = -2 \cdot \log L + k \cdot \log(n)
\end{equation}
donde $\log L$ es la log-verosimilitud del modelo, $k$ el número de parámetros libres, y $n$ el número de observaciones. Un menor BIC indica un mejor equilibrio entre ajuste y complejidad.

\subsection{Enfoque 1: Chow-Liu}

El algoritmo de Chow-Liu (1968) construye la red Bayesiana en dos pasos: (1) calcular la Información Mutua entre todos los pares de variables, y (2) encontrar el Árbol de Expansión de Peso Máximo (MST), que es exactamente el procedimiento realizado en las secciones anteriores. La estructura resultante es un árbol dirigido desde el nodo Target como raíz.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{dag_chow_liu.png}
    \caption{Red Bayesiana -- Chow-Liu (MST sobre MI). Target como raíz.}
\end{figure}

\subsection{Enfoque 2: Naive Bayes}

El modelo Naive Bayes asume independencia condicional entre todas las features dado el Target. Su estructura es la más simple posible: el Target apunta a cada una de las 9 variables predictoras, sin aristas entre features.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{dag_naive_bayes.png}
    \caption{Red Bayesiana -- Naive Bayes. Target $\rightarrow$ features, sin aristas entre predictores.}
\end{figure}

\subsection{Enfoque 3: Hill Climbing}

El algoritmo Hill Climbing realiza una búsqueda heurística en el espacio de DAGs, partiendo de un grafo vacío y añadiendo, eliminando o invirtiendo aristas para maximizar el score BIC. A diferencia de Chow-Liu (restringido a árbol) y Naive Bayes (restringido a estructura estrella), Hill Climbing puede descubrir cualquier estructura de grafo.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{dag_hill_climbing.png}
    \caption{Red Bayesiana -- Hill Climbing. Estructura libre optimizada por BIC.}
\end{figure}

\subsection{Comparativa de los Tres Enfoques}

\begin{table}[H]
    \centering
    \begin{tabular}{lcccc}
        \toprule
        \textbf{Modelo} & \textbf{Aristas} & \textbf{BIC} & \textbf{Log-Lik} & \textbf{Parámetros} \\
        \midrule
        Chow-Liu & 9 & 63,310.67 & -31,631.86 & 47 \\
        Naive Bayes & 9 & 69,667.01 & -34,809.86 & 47 \\
        Hill Climbing & 23 & 68,056.57 & -33,565.87 & 737 \\
        \midrule
        \rowcolor{blue!15}
        \textbf{Mejor (Chow-Liu)} & 9 & \textbf{63,310.67} & -31,631.86 & 47 \\
        \bottomrule
    \end{tabular}
...
Los tres enfoques revelan fortalezas y limitaciones del aprendizaje estructural:

\begin{itemize}
    \item \textbf{Chow-Liu es el mejor modelo} según el criterio BIC (63,310.67). Con solo 9 aristas y 47 parámetros, captura las dependencias esenciales entre variables sin sobreajustar. La restricción de estructura de árbol resulta ser una ventaja: evita la inclusión de aristas espurias que inflan la complejidad sin mejorar el ajuste.

    \item \textbf{Naive Bayes es el peor modelo} (BIC = 69,667.01). Aunque tiene el mismo número de parámetros que Chow-Liu, su suposición de independencia condicional entre features es demasiado restrictiva. La realidad es que existen dependencias significativas entre variables como $x_5$ (1er semestre) y $x_7$ (2do semestre), que Naive Bayes ignora por completo.

    \item \textbf{Hill Climbing sobreajusta} (BIC = 68,056.57). Con 23 aristas y 737 parámetros, descubre muchas más relaciones, pero la penalización por complejidad del BIC castiga este exceso. El resultado es un modelo peor que Chow-Liu a pesar de ser mucho más complejo. Esto ilustra el principio de parsimonia: más aristas no implican necesariamente un mejor modelo.

    \item \textbf{La penalización por complejidad es crucial}: Chow-Liu y Naive Bayes tienen la misma cantidad de aristas (9), pero Chow-Liu distribuye sus 47 parámetros de manera más efectiva al capturar dependencias entre features. Hill Climbing, con 737 parámetros, distribuye la información en demasiadas aristas, muchas de ellas probablemente espurias.
\end{itemize}

\subsection{Tablas de Probabilidad Condicional (Chow-Liu)}

Para el modelo ganador (Chow-Liu), se presentan las CPTs de los nodos conectados directamente al Target:
...
\begin{table}[H]
    ...
    \begin{tabular}{c|ccc}
        ...
        \bottomrule
    \end{tabular}
    \caption{CPT: $P(x_7 \mid Target)$ -- Aprobados 2do semestre.}
\end{table}

Interpretación: Si un estudiante abandona, hay un $82.2\%$ de probabilidad de bajo rendimiento en 2do semestre. Si se gradúa, solo el $11.6\%$.
...
\begin{table}[H]
    ...
    \begin{tabular}{c|cc}
        ...
        \bottomrule
    \end{tabular}
    \caption{CPT: $P(x_3 \mid Target)$ -- Pagos al día.}
\end{table}
...
La combinación de baja nota de admisión y falta de pago predice abandono con más del $92\%$ de probabilidad.

\subsection{Conclusiones de las Redes Bayesianas}

\begin{itemize}
    \item El algoritmo de \textbf{Chow-Liu} produce el mejor DAG según BIC, validando que la estructura de árbol aprendida desde la matriz de Información Mutua es óptima para este dataset.

    \item La comparación de tres enfoques revela un \textbf{principio fundamental}: restricciones estructurales adecuadas (árbol) producen mejores modelos que la libertad total (Hill Climbing) cuando los datos son limitados. La penalización por complejidad del BIC previene el sobreajuste.

    \item Las CPTs del modelo Chow-Liu permiten \textbf{cuantificar el riesgo} de abandono para perfiles específicos, con aplicaciones directas en sistemas de alerta temprana universitaria.

    \item La red Bayesiana obtenida es un modelo \textbf{explicable e interpretable} con solo 9 aristas dirigidas, donde cada arco representa una dependencia probabilística con sentido en el contexto educativo.
\end{itemize}
